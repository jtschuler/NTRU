...
class Encrypt:
    ...
    def encrypt(self, message, r = None):
        ...
        e = polynomials.multiplication(r, self.key.h, self.ntru.q)
...
class Keys:
    """
    >>> message = [4,5,6,7,1,2,3,0]; ntru = NTRU(2,2,2,257,259,8); key = Keys(ntru); encrypt=Encrypt(key, ntru); e = encrypt.encrypt(message); d = key.decrypt(e); print(e, d, message); d == message
    True
    """
    def __init__(self, ntru, f=None, g=None):
        ...
        while True:
            try:
                ...
                self.h = polynomials.multiplication(self.Fq, self.g, self.q, self.N)
                if (polynomials.trim(self.h) == []):
                    continue
                if(polynomials.trim(polynomials.multiplication(self.Fp, self.f, self.p)) == [1]):
                    return
